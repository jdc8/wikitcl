...
proc parse_command_line {} {
    ...
    set tcltkdir $::nroffsubdir
    set tkdir {}
    set tcldir {}
    set webdir $::nroffsubdir
    set build_tcl 0
    set build_tk 0
...
proc long-toc {text} {
    ...
    lappend manual(section-toc) \
	    "<DD><A HREF=\"#$here\" NAME=\"$there\">$text</A>"
...
proc option-toc {name class switch} {
    ...
    lappend manual(section-toc) \
	"<DD><A HREF=\#$here\" NAME=\"$there\">$switch, $name, $class</A>"
...
proc std-option-toc {name page} {
    ...
    set there L[incr manual(long-toc-n)]
    set other M$name
    lappend manual(section-toc) "<DD>$name" ;#"<DD><A HREF=\"$page.htm#$other\">$name</A>"
    return $name ;# "<A HREF=\"$page.htm#$other\">$name</A>"
}
##
...
if {1} {
    if {[catch {
	make-man-pages $webdir [list $::nroffsubdir/tnr.n "Nroff2Wiki" "Nroff2Wiki" ""]
    } error]} {
	puts $error\n$errorInfo
    }
}
